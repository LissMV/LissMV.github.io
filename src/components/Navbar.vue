...
<template>
  <div>
    <nav
      class="backdrop-blur-sm fixed top-1/2 left-16 -translate-y-1/2 w-[70px] bg-gradient-to-b from-gris-claro/20 via-cyan/20 to-cyan/20 rounded-lg shadow-lg flex flex-col items-center py-3 gap-10 z-50"
      style="backdrop-filter: blur(10px);">
      <button v-for="item in navItems" :key="item.id" @click="handleClick(item.id, item.route)" :class="[
        ...
        <component :is="item.icon" :fill="getIconColor(item.id)" class="transition-all duration-200" />
      </button>
    </nav>

    <ProjectsPopup :isOpen="showProjectsPopup" @close="showProjectsPopup = false" />
  </div>
</template>
